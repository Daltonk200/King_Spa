  ...
  return (
    <div className="grid grid-cols-3 md:grid-cols-4 gap-4 h-96">
      ...
      <div
        ...
        data-aos="fade-up"
      >
        <a href={images[0].src} className="glightbox">
          <img
            src={images[0].src}
            alt={images[0].alt}
      ...
      {images.slice(1).map((image, index) => (
        <div
          key={index}
          className="w-full h-auto overflow-hidden" // overflow-hidden to hide zoomed image overflow
          data-aos="fade-up"
        >
          <a href={image.src} className="glightbox">
            <img
              src={image.src}
              alt={image.alt}
              className="w-full h-full object-cover rounded-lg transition-transform duration-300 ease-in-out hover:scale-110" // Zoom effect using Tailwind
